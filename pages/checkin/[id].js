...
// Calculate elapsed minutes — handles midnight crossover
function calcElapsedMins(checkInDate, checkInTime) {
  if (!checkInDate || !checkInTime) return 0
  const inDT = new Date(`${checkInDate}T${checkInTime}`)
  const nowDT = new Date(new Date().toLocaleString('en-US', { timeZone: TIMEZONE }))
  return Math.max(0, Math.round((nowDT - inDT) / 60000))
}

// Calculate session minutes between two date+time pairs
...
export default function CheckIn() {
  ...
  useEffect(() => {
    ...
    const update = () => setElapsedMins(calcElapsedMins(openEntry.date, openEntry.check_in))
    update()
  ...
  async function handleCheckIn() {
    ...
    const { error } = await supabase.from('entries').insert({
      employee_id: id, date: todayStrJO(), check_in: timeStrJO(), minutes: 0
    })
  ...
  async function handleCheckOut() {
    ...
    const outTime = timeStrJO()
    // Use date-aware calculation to handle midnight crossover
    const minutes = calcSessionMins(openEntry.date, openEntry.check_in, outDate, outTime)
  ...
  const todayEntries = allEntries.filter(e => e.date === today || e.check_out_date === today)
  const todayCompletedMins = allEntries
    .filter(e => e.check_out && (e.date === today || e.check_out_date === today))
    .reduce((s, e) => s + (e.minutes || 0), 0)
  const todayTotalMins = todayCompletedMins + (openEntry ? elapsedMins : 0)

  const weekEntries = allEntries.filter(e => e.date >= wStart || (e.check_out_date && e.check_out_date >= wStart))
  ...
  const todaySessionList = allEntries.filter(e => e.date === today).slice().reverse()

  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', flexDirection:'column', gap:12 }}>
      <div style={{ fontSize:'2rem' }}>⏳</div><div style={{ color:'#9ca3af' }}>جارٍ التحميل...</div>
    </div>
  )
  if (!employee) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', flexDirection:'column', gap:12 }}>
      <div style={{ fontSize:'3rem' }}>❌</div><div style={{ color:'#e24b4a', fontWeight:600 }}>لم يتم العثور على الموظف</div>
    </div>
  )
    ...
    <>
      ...
      <div className="container" style={{ paddingTop:'1.5rem', paddingBottom:'3rem' }}>
        ...
        <div className="card checkin-card">
          ...
          <div style={{ textAlign:'center', padding:'0.85rem 1rem', background:openEntry?'#e1f5ee':'#f9fafb', borderRadius:10, marginBottom:'1.25rem', border:`1px solid ${openEntry?'#9fe1cb':'#e5e7eb'}` }}>
            {openEntry ? (
              <>
                <div style={{ fontWeight:700, color:'#085041' }}>✅ مسجّل دخول منذ {openEntry.check_in?.slice(0,5)} ({openEntry.date === today ? 'اليوم' : 'أمس'})</div>
                <div style={{ fontSize:'1.4rem', fontWeight:700, color:'#1d9e75', marginTop:4, fontVariantNumeric:'tabular-nums' }}>⏱ {fmtHours(elapsedMins)} هذه الجلسة</div>
              </>
            ) : (
          ...
          {(openEntry || todayCompletedMins > 0) && (
            <div style={{ background:'#085041', color:'white', borderRadius:10, padding:'0.75rem', textAlign:'center', marginBottom:'1rem' }}>
              <div style={{ fontSize:'0.78rem', opacity:0.8, marginBottom:2 }}>إجمالي اليوم</div>
              <div style={{ fontSize:'2rem', fontWeight:700, fontVariantNumeric:'tabular-nums' }}>{fmtHours(todayTotalMins)}</div>
            </div>
          )}

          <div style={{ display:'flex', gap:10 }}>
            <button className="btn btn-primary btn-lg" onClick={handleCheckIn} disabled={working||!!openEntry} style={{ flex:1, opacity:openEntry?0.4:1 }}>
              🟢 تسجيل دخول
            </button>
            <button className="btn btn-lg" onClick={handleCheckOut} disabled={working||!openEntry}
          ...
          {todaySessionList.length > 0 && (
            <div style={{ marginTop:'1.25rem', borderTop:'1px solid #f3f4f6', paddingTop:'1rem' }}>
              ...
              {todaySessionList.map((e, i) => (
                <div key={e.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0.4rem 0.6rem', background:'#f9fafb', borderRadius:6, marginBottom:4 }}>
                  <span style={{ fontSize:'0.85rem', color:'#374151' }}>
                    ...
                    {e.check_out_date && e.check_out_date !== e.date ? ' (اليوم التالي)' : ''}
                  </span>
                  {e.check_out
              ...
              <div style={{ marginTop:'0.5rem', display:'flex', justifyContent:'space-between', fontWeight:700, fontSize:'0.9rem', color:'#085041', padding:'0.4rem 0.6rem', background:'#e1f5ee', borderRadius:6 }}>
                <span>مجموع اليوم:</span><span>{fmtHours(todayTotalMins)}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ maxWidth:480, margin:'0 auto 1.25rem' }}>
          ...
          {summaryMode==='today' && (
            <div className="stats-grid">
              <div className="stat-card"><div className="stat-value" style={{ fontVariantNumeric:'tabular-nums' }}>{fmtHours(todayTotalMins)}</div><div className="stat-label">إجمالي اليوم</div></div>
              <div className="stat-card"><div className="stat-value">{todaySessionList.length}</div><div className="stat-label">عدد الجلسات</div></div>
            </div>
          )}
          {summaryMode==='week' && (
            <div className="stats-grid">
              <div className="stat-card"><div className="stat-value" style={{ fontVariantNumeric:'tabular-nums' }}>{fmtHours(weekTotalMins)}</div><div className="stat-label">إجمالي الأسبوع</div></div>
              <div className="stat-card"><div className="stat-value">{weekDays}</div><div className="stat-label">أيام العمل</div></div>
            </div>
          )}
          {summaryMode==='month' && (
            <div className="stats-grid">
              <div className="stat-card"><div className="stat-value" style={{ fontVariantNumeric:'tabular-nums' }}>{fmtHours(monthTotalMins)}</div><div className="stat-label">إجمالي الشهر</div></div>
                ...
                <div key={d} style={{ marginBottom:'1rem', paddingBottom:'1rem', borderBottom:'1px solid #f3f4f6' }}>
                  ...
                  {dayEntries.slice().reverse().map((e,i) => (
                    <div key={e.id} style={{ display:'flex', justifyContent:'space-between', fontSize:'0.8rem', color:'#6b7280', padding:'0.25rem 0.5rem', background:'#f9fafb', borderRadius:5, marginBottom:2 }}>
                      <span>جلسة {i+1}: {e.check_in?.slice(0,5)} ← {e.check_out ? e.check_out.slice(0,5) : '...'}{e.check_out_date&&e.check_out_date!==e.date?' (+يوم)':''}</span>
